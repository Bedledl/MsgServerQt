...
{
    mutex.lockForRead();
    std::for_each(messages.begin(), messages.end(), std::move(func));
    mutex.unlock();
}
...
void Chat::addMessage(Message msg)
{
    ...
    if (messages.empty() || msg > getLastMessage())
    {
...
{
    addMessage(Message(std::move(content), std::move(sender), std::move(timestamp)));
}
...
void Chat::removeParticipant(const std::shared_ptr<Participant> &participant)
{
    mutex.lockForWrite();
...
const Message &Chat::getMessageAt(size_t index) const
{
    ...
    const auto &r = *std::next(messages.begin(), index);
    mutex.unlock();
    return r;
}
